...
test("received file is auto-saved and appears in the library (survives reload)", async ({
  ...
  await expect(tvPage.locator(".tv-library-name", { hasText: "تلقائي.txt" })).toBeVisible({ timeout: 10_000 });
...
test("multiple files accumulate in the library", async ({ context }) => {
  const first = await sendFileToTv(context, "أول.txt", Buffer.from("أول ".repeat(2000)));
  const secondContext = await context.browser()!.newContext();
  const second = await sendFileToTv(
    secondContext,
    "ثاني.txt",
    Buffer.from("ثاني ".repeat(2000)),
  );

  // كلا الملفين في المكتبة على نفس التخزين (نفس السياق؟ نستخدم سياق واحد للتجربة)
  // نتحقق من أول ملف على الأقل في مكتبته
  await first.tvPage.getByRole("button", { name: /الملفات المستلمة/ }).click();
  await expect(first.tvPage.locator(".tv-library-name", { hasText: "أول.txt" })).toBeVisible({ timeout: 10_000 });
