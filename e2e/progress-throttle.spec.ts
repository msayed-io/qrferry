...
test("progress updates are throttled (not per-chunk) during a large transfer", async ({
  ...
  await tvPage.evaluate(() => {
    ...
    const observer = new MutationObserver(() => {
      const text = document.querySelector(".tv-progress-text")?.textContent;
      if (text && text !== previous) {
        previous = text;
        (
          window as unknown as { __progressUpdates: number }
        ).__progressUpdates += 1;
      }
    });
    // The progress element is created only after receipt starts: observe its parent.
    ...
    (
      window as unknown as { __progressObserver?: MutationObserver }
    ).__progressObserver = observer;
  });
